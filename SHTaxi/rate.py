...
def retrieveRows(startTime, endTime):
  ...
  for csvFile in os.listdir(dataPath):
    ...
    gridCoor(gridSet, selRows, m, n)
    candidates[csvFile[7:-4]] = gridSet
    #if the coordinates of GPS is out of range, print its rows
    # print "File Name is: %s, rows that are OK:%s" % (csvFile,selRows)
    print "File Name is: %s, rows that exceed boundary:%s" % (csvFile,noOKRows)

  return candidates, m*n    
   
def selectKTaxis(K, candidates, cntOfGrid):
  AccuSetData = set()
  taxiList = []
  if K >= len(candidates.keys()):
    ...
    while(len(taxiList)<K):
      ...
      # print AccuSetData
      # print "numOfGrid:%d" % cntOfGrid
  return taxiList, len(AccuSetData) / float(cntOfGrid)

def calculateHours(st,et):
  return (et - st).seconds / 3600

def main():
  startTime = '2007-02-01T07:00:00+08:00'
  endTime = '2007-02-01T09:00:00+08:00'
  
  hours = calculateHours(turnStrToTime(startTime), turnStrToTime(endTime))

  cand, cntOfGrid = retrieveRows(startTime, endTime)
  #print len(cand.values()[0]),len(cand.values()[0][0])

  #draw Pic 
  KList = [i for i in range(1,201)]
  covRates = []
  ...
  for K in KList:
    ...
    taxiList, covRate = selectKTaxis(K,candidates, cntOfGrid)
    covRates.append(covRate)
    
    print "K=%s,covRate=%s" % (K,covRate)
    print "taxiList:%s" % ','.join(taxiList)

  plt.plot(np.array(KList),np.array(covRates),'-o') 
